...
<head>
    ...
    <title>Ingredientes - Della Massa</title>
</head>
<body id="padrao">
    ...
    <main>
        <div>
            <form action="ingrediente.php" method="GET">
            ...
            <table class="table table-striped" id='conteudo'>
                ...
                <?php
                    ...
                    $base = $pag * $base2;
                    if($pesquisar == "") {
                        $comando = "SELECT * FROM ingrediente ORDER BY id_ingrediente DESC LIMIT $base, $base2";
                        $comandoCount = "SELECT COUNT(*) FROM ingrediente";
                    } else {
                        $comando = "SELECT * FROM ingrediente WHERE nome LIKE '%".$pesquisar."%' ORDER BY id_ingrediente DESC LIMIT $base, $base2";
                        $comandoCount = "SELECT COUNT(*) FROM ingrediente WHERE nome LIKE '%".$pesquisar."%' LIMIT $base, $base2";
                    }
                    ...
                    while ($campo=mysqli_fetch_array($result)){
                        ...
                        $tabela->insere_coluna($campo["id_ingrediente"],$contador,0);
                        $tabela->insere_coluna($campo["nome"],$contador,1);
                        $tabela->insere_coluna($campo["valor"],$contador,2);
                        $tabela->insere_coluna3("",$contador,3,"Inserir", "formIngrediente","");
                        $tabela->insere_coluna3($campo["id_ingrediente"],$contador,4,"Editar", "formIngrediente","");
                        $tabela->insere_coluna3($campo["id_ingrediente"],$contador,5,"Excluir", "formIngrediente","");
                        $tabela->fecha_linha();
                        $contador++;
                    }
                ?>
